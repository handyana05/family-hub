      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
        <section className="rounded-3xl bg-slate-100 p-6 ring-1 ring-slate-200 dark:bg-white/5 dark:ring-white/10">
          <div className="mb-4 flex items-center justify-between gap-4">
            <h2 className="text-3xl font-semibold text-slate-950 dark:text-white">Month view</h2>
            <WallSwipeHint />
          </div>

          <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-white/10">
            <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-200 dark:border-white/10 dark:bg-white/5">
              {WEEKDAY_LABELS.map((label) => (
                <div
                  key={label}
                  className="p-4 text-lg font-medium text-slate-500 dark:text-slate-300"
                >
                  {label}
                </div>
              ))}
            ...
            <div className="divide-y divide-slate-200 dark:divide-white/10">
              {weeks.map((week, index) => (
                <div
                  key={index}
                  className="grid grid-cols-7 divide-x divide-slate-200 dark:divide-white/10"
                >
                  {week.days.map((day) => (
                    <Link
                      key={day.key}
                      ...
                      className={`block min-h-40 p-3 transition ${
                        ...
                        day.isSelected
                          ? "ring-2 ring-amber-400 ring-inset bg-amber-50 dark:bg-white/10"
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          {day.eventCount > 0 ? (
                            <span className="rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-medium text-sky-700 ring-1 ring-sky-200 dark:bg-sky-500/20 dark:text-sky-200 dark:ring-sky-400/20">
                              {day.eventCount}E
                            </span>
                          ) : null}

                          {day.dueTodoCount > 0 ? (
                            <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-700 ring-1 ring-amber-200 dark:bg-amber-500/20 dark:text-amber-200 dark:ring-amber-400/20">
                              {day.dueTodoCount}T
                            </span>
                          ) : null}
                        </div>
        ...
        <aside className="rounded-3xl bg-slate-100 p-6 ring-1 ring-slate-200 dark:bg-white/5 dark:ring-white/10">
          <h2 className="text-3xl font-semibold text-slate-950 dark:text-white">Selected day</h2>
          <p className="mt-2 text-xl text-slate-500 dark:text-slate-300">
            {format(selectedDate, "EEEE, MMMM d, yyyy")}
          </p>

          <div className="mt-6 space-y-6">
            <section>
              <h3 className="mb-3 text-2xl font-semibold text-slate-950 dark:text-white">Events</h3>
            ...
            <section>
              <h3 className="mb-3 text-2xl font-semibold text-slate-950 dark:text-white">
                Todos due that day
              </h3>
              {selectedDayOpenTodos.length === 0 ? (
                ...
                <div className="space-y-3">
                  {selectedDayOpenTodos.map((todo) => (
                    <div
                      key={todo.id}
                      className="flex items-center justify-between gap-3 rounded-2xl bg-white px-4 py-3 ring-1 ring-slate-200 dark:bg-white/5 dark:ring-white/10"
                    >
                      <div>
              ...
              {selectedDayCompletedTodos.length === 0 ? (
                ...
                <div className="space-y-3">
                  {selectedDayCompletedTodos.map((todo) => (
                    <div
                      key={todo.id}
                      className="flex items-center justify-between gap-3 rounded-2xl border border-emerald-300 bg-emerald-50 px-4 py-3 dark:border-emerald-400/20 dark:bg-emerald-500/10"
                    >
                      <div>
                        <p className="text-xl font-medium text-emerald-800 dark:text-emerald-100">
                          {todo.title}
                        </p>
                        <p className="mt-1 text-base text-emerald-700 dark:text-emerald-200/80">
